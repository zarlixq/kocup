...
import { PaymentsFilters } from "@/blocks/admin/payments-filters"
...
export default async function OdemelerPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">Tüm ödemeleri görüntüle ve filtrele.</p>
        </div>
      ...
      {!payments || payments.length === 0 ? (
        <div className="bg-white border border-zinc-200 rounded-2xl p-12 text-center">
          ...
          <h3 className="text-base font-semibold text-zinc-900 mb-1">Bu filtreyle ödeme bulunamadı</h3>
          <p className="text-sm text-zinc-500">Farklı bir ay veya öğrenci seçmeyi dene.</p>
        </div>
      ) : (

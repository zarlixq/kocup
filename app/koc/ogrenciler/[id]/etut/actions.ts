...
export async function createEtutEntry(
  studentId: string,
  input: unknown,
): Promise<Result> {
  ...
  if (conflict) {
    return { success: false, error: "Bu gün ve saat aralığında başka bir etüt var." }
  }

  const { error } = await auth.supabase.from("etut_schedule").insert({
  ...
  if (error) {
    ...
    return { success: false, error: "Etüt eklenemedi, tekrar deneyin." }
  }

  revalidatePath(`/koc/ogrenciler/${studentId}/etut`)
  revalidatePath(`/mudur/ogrenciler/${studentId}/etut`)
  return { success: true }
}

export async function updateEtutEntry(
  studentId: string,
  entryId: string,
  input: unknown,
): Promise<Result> {
  ...
  if (error) {
    console.error("Etüt güncelleme hatası:", error)
...
export async function deleteEtutEntry(
  studentId: string,
  entryId: string,
): Promise<Result> {
  const auth = await requireCoachOwnership(studentId)
  ...
  if (error) {
    console.error("Etüt silme hatası:", error)

...
export async function createPaymentAction(studentId: string, formData: FormData) {
  ...
  const period_month = s(formData.get("period_month"))
...
export async function deletePaymentAction(paymentId: string) {
  ...
  if (error) return { error: error.message }
  if (data?.student_id) {
    revalidatePath(`/koc/ogrenciler/${data.student_id}`)
    revalidatePath(`/koc/ogrenciler/${data.student_id}/odemeler`)
  }
  revalidatePath("/koc/payments")
  revalidatePath("/koc")
  return { ok: true }

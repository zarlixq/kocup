...
/**
 * Koç ve müdür/org_admin panellerinde paylaşılan ZENGİN öğrenci genel bakış görünümü.
 * variant="coach": kendi öğrencisi — düzenleme detay layout'ta.
 * variant="readonly": müdür/org_admin — sadece görüntüleme; atanan koç + readonly banner gösterilir.
 */
export async function StudentOverview({
  ...
  studentId: string
  variant: "coach" | "readonly"
  appointmentsHref: string
  /** readonly modda atanan koça link için (örn. "/mudur/koclar"). */
  coachHrefPrefix?: string
}) {
  const id = studentId
          ...
          <div className="md:col-span-3 pt-3 border-t border-zinc-100 flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              {coach ? (
                coachHrefPrefix ? (
                  <Link
                    href={`${coachHrefPrefix}/${coach.id}`}
                    className="text-sm font-medium text-[#1B6B8A] hover:underline"
                  >
                    {coach.full_name}
                  </Link>
                ) : (
                  <div className="text-sm font-medium text-zinc-900">{coach.full_name}</div>
                )
              ) : (
          ...
          appointment={
            upcomingAppt
              ? {
                  ...
                  otherPersonName: readonly ? coach?.full_name ?? null : profile.full_name,
                }
              : null
          }
          href={appointmentsHref}
          emptyText="Yaklaşan randevu yok"
          emptyHref={appointmentsHref}
        />
      </div>

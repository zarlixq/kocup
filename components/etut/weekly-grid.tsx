...
import {
  EtutFormDialog,
  type SubjectOption,
} from "@/components/etut/etut-form-dialog"
import { deleteEtutEntry } from "@/app/koc/ogrenciler/[id]/etut/actions"

export type EtutEntryWithSubject = {
  id: string
  term: number
  day_of_week: number
  start_time: string
...
function colorFor(entry: EtutEntryWithSubject): string {
...
export function WeeklyEtutGrid({
  entries,
  subjects,
  editable,
  studentId,
  ...
  const [editTarget, setEditTarget] = useState<EtutEntryWithSubject | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<EtutEntryWithSubject | null>(null)
  const [deletePending, setDeletePending] = useState(false)

  const byDay = useMemo(() => {
    const map = new Map<number, EtutEntryWithSubject[]>()
  ...
  async function handleDelete() {
    ...
    const res = await deleteEtutEntry(studentId, deleteTarget.id)
    setDeletePending(false)
    if (res.success) {
      toast.success("Etüt silindi.")
    ...
    <>
      ...
      {editable && (
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Etüdü sil?</AlertDialogTitle>
              <AlertDialogDescription>
                <span className="font-medium">
                  {deleteTarget?.subject?.name ?? deleteTarget?.custom_title ?? "Bu etüt"}
                </span>{" "}
                etüt programından kalıcı olarak silinecek. Bu işlem geri alınamaz.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
...
type ColumnProps = {
  ...
  isToday: boolean
  onAdd: () => void
  onEdit: (e: EtutEntryWithSubject) => void
  onDelete: (e: EtutEntryWithSubject) => void
      ...
      <div className={cn("flex-1 p-2 space-y-1.5", isMobile && "pt-3")}>
        {entries.length === 0 ? (
          <div className="h-full min-h-[120px] flex items-center justify-center">
            <p className="text-xs text-zinc-400 italic">Etüt yok</p>
          </div>
        ) : (
          entries.map((e) => (
            <LessonCard
